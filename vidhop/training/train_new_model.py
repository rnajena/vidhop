...
@click.command()
@click.option('--inpath', '-i', required=True,help='path to the dir with training files, renerated with make_dataset.py')
...
def cli(inpath, outpath, name, epochs, architecture, extention_variant, early_stopping, repeated_undersampling):
	'''
		Example:
...
# general_setting = {"path": inpath, "suffix": "model7_deepertf.keras.layers.Dense_ROC", "sampleSize": 1,
...
# test_and_plot(path=inpath, suffix=name)
...
class prediction_history(tf.keras.callbacks.Callback):
	...
	def on_epoch_end(self, epoch, logs={}):
		...
		# print(tf.keras.backend.get_value(self.model.optimizer.iterations))
...
def acc_with_cutoffs_per_class(Y_true, Y_pred, cutoffs):
	...
	acc = sum(Y_true[prediction_mask] == Y_pred_bin[prediction_mask]) / Y_true[prediction_mask].size  #same as below but not as intuitive
...
def use_data_nanocomb(directory, one_hot_encoding=True, repeat=True, use_spacer=True, maxLen=0, online=False,
	...
	if one_hot_encoding:
		...
		X_train = DataParsing.encode_string(maxLen=maxLen, x=X_train_old, repeat=repeat, use_spacer=use_spacer,
											online_Xtrain_set=online)
		X_test = DataParsing.encode_string(maxLen=maxLen, x=X_test_old, repeat=repeat, use_spacer=use_spacer)
		if create_val == False:
			X_val = DataParsing.encode_string(maxLen=maxLen, x=X_val_old, repeat=repeat, use_spacer=use_spacer)
	else:
		X_train = X_train_old
		X_test = X_test_old
		if create_val == False:
...
def model_for_plot(inpath, outpath, design=1, sampleSize=1, nodes=32, suffix="", epochs=100, dropout=0,
	...
	if design == 5:
		...
		# model.add(layers.tf.keras.layers.MaxPooling1D(3))
	...
	if use_generator:
		...
		params = {"number_subsequences": number_subsequences, "dim": timesteps, "n_channels": X_test.shape[-1], "number_samples_per_batch": batch_size,
		...
		if online_training == True:
			...
			for epo in range(epochs):
				X_train_manipulated, Y_train_manipulated = DataParsing.manipulate_training_data(X=X_train_copy_clean,
																								Y=Y_train,
...
def calc_predictions(X, Y, y_pred, do_print=False):
	...
	for i in range(0, int(len(y_pred) / number_subsequences)):
		sample_pred_mean = np.array(np.sum(y_pred[i * number_subsequences:i * number_subsequences + number_subsequences], axis=0) / number_subsequences)
		...
		y_pred_mean_weight_ent.append(np.argmax(np.array(
			np.sum(np.array(y_pred[i * number_subsequences:i * number_subsequences + number_subsequences]) * sw_normalized, axis=0) / number_subsequences)))
	...
	for i in range(0, int(len(y_true) / number_subsequences)):
		...
		y_pred_voted.append(np.random.permutation(best)[0]) #if (very unlikely) multiple classes have the same vote prob. take randomly one

		y_true_small.append(np.argmax(np.array(np.bincount(y_true[i * number_subsequences:i * number_subsequences + number_subsequences]))))
...
def test_and_plot(inpath, outpath, suffix, batch_norm=False, filter_trainset=False, use_old_dataset=False,
	...
	if do_shrink_timesteps:
		...
		if not online_training:
			X_train, Y_train, number_subsequences = DataParsing.shrink_timesteps(X_train, Y_train, input_subSeqlength)
		X_val, Y_val, number_subsequences = DataParsing.shrink_timesteps(X_val, Y_val, input_subSeqlength)
		X_test, Y_test, number_subsequences = DataParsing.shrink_timesteps(X_test, Y_test, input_subSeqlength)
	else:
	...
	for model_path in (model_path1, model_path2):
		...
		pickle.dump((model.to_json(),model.get_weights(),index_classes,multi_thresh, maxLen, repeat, use_repeat_spacer,
